...
if rank(CC)<n_endog,
  ...
  warnings = [warnings;message];
    ...
    if l_equ == 0,
        ...
        if DO_USUAL_XI_DELTA,
           Xi_mat    = [ Gamma_mat,     Theta_mat
               eye(m_states), zeros(m_states) ];
           Delta_mat = [ Psi_mat,       zeros(m_states)
               zeros(m_states), eye(m_states) ];
        else  % This is an alternative that should work equally well.
            % It is offered here on an experimental basis for problems
            % with the impossibility of solving for Omega_mat, 
            % but it may not help as it is
            % unlikely to change any results from here on.
           REG_FACTOR = 1;
           RAND_FACTOR = 0.1;
           ADD_MATRIX = REG_FACTOR*eye(m_states) + RAND_FACTOR*rand(m_states,m_states);
           Xi_mat    = [ zeros(m_states)+ADD_MATRIX,  Theta_mat
               eye(m_states), zeros(m_states) ];
           Delta_mat = [ Psi_mat,   ADD_MATRIX-Gamma_mat
               zeros(m_states), eye(m_states) ];
        end;
 end;
  if DO_QZ,
     ...
     if rank(Xi_eigvec)<m_states,
       ...
       SOLVE_FLAG_PP = (rank(Omega_mat)<m_states);
       if SOLVE_FLAG_PP,
         ...
         PP_imag = imag(PP);
         PP = real(PP);
         SOLVE_FLAG_COMPLEX = sum(sum(abs(PP_imag))) / sum(sum(abs(PP))) > .000001;
         if SOLVE_FLAG_COMPLEX,
