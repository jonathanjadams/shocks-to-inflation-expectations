...
%   DO_QUIET   = 0;    % Set = 1, if you do not want any messages, i.e. if you want the
%                      % program to run as quietly as possible.  Useful e.g. for numerical searches.
%                      % DO_QUIET = 1 sets a number of options to display stuff to "don't display".
%                      % Note: impulse responses will still be produced, turn them off with DO_IMPRESP = 0;
%   DO_SOL_OUT = 1;    % Prints the results from solving for the recursive law of motion.
...
%   DO_MOM_OUT = 1;    % Prints the results, if DO_MOMENTS = 1
%   DO_SIMUL = 0;      % Turn on, if you want to do simulations or 
%                      % simulation-based calculations of moments
%   DO_SIM_OUT = 1;    % Prints the results, if DO_SIMUL = 1;
%   DO_IGNORE_FLAGS =1;% Solve sets three flags:
%                      % SOLVE_FLAG_PP: if there are problems in computing PP
%                      % SOLVE_FLAG_COMPLEX: if PP is complex
%                      % SOLVE_FLAG_VV: if there are problems computing VV
%                      % SOLVE_FLAGS = 1, if one of these = 1.
%                      % DO_IT stops after that, if IGNORE_FLAGS = 0;
%                      % IT always stops, if SOLVE_FLAG_PP = 1;
%                      % Normally, it proceeds.
%
...
%============> GENERAL OPTIONS:
if exist('DO_QUIET')~=1,
    DO_QUIET = 0; % Prints the calculated moments, if DO_MOMENTS = 1.
end;
if DO_QUIET,  % Overriding some defaults:
    DO_DISPLAY_IMMEDIATELY = 0;
    DO_SOL_OUT = 0;
    DO_MOM_OUT = 0;
    DO_SIM_OUT = 0;
end;
if ~exist('DO_IGNORE_FLAGS'),
    DO_IGNORE_FLAGS = 1;
end;
if exist('PERIOD')~=1,
    PERIOD     = 4;  % number of periods per year, i.e. 12 for monthly, 4 for quarterly
...
%============> options for DO_IT.M
if exist('DO_SOL_OUT')~=1,
    DO_SOL_OUT = 1; % Prints the results from solving the model
end;
...
if exist('DO_MOM_OUT')~=1,
    DO_MOM_OUT = 1; % Prints the calculated moments, if DO_MOMENTS = 1.
end;
if DO_MOMENTS==1,
...
if exist('DO_SIM_OUT')~=1,
    DO_SIM_OUT = 1; % Prints the simulated moments, if DO_SIMUL = 1.
end;
if DO_SIMUL,
...
if exist('DO_QZ')~=1,
    DO_QZ = 1; % = 1, if you want to calculate the P matrix with
    % the QZ-algorithm, see SOLVE_QZ.M.
    % The QZ-algorithm is probably numerically more
    % stable and deals better with repeated eigenvalues.
end;
